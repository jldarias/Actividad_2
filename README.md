...
Partiendo de la actividad 1 que consistía en la realización de una aplicación de medición y presentación de datos del clima, en la que manejamos sensores de temperatura, humedad, iluminación, calidad de aire, velocidad y dirección del viento de una estación meteorológica (boya climática), procesamos la medición con instrumentación programable (Arduino UNO) y presentamos los resultados en una HMI local (Display LCD I2C). En la presente actividad, que es una continuación de la anterior, añadimos actuadores y algoritmos de control que, a partir de la medición de los sensores, nos permitirán mantener el sistema cercano a los valores deseados y presentar los resultados en la HMI local.

También se han corregido algunos errores y realizado algunas modificaciones en la actividad 1, las cuales describimos a continuación:
- Cambiamos el tipo de variable byte de temperature y humidity a tipo float, ya que al simular el sistema con valores de temperatura negativos, el display devolvía valores fuera de rango.
- Creamos dos librerías: LCD.cpp que contiene el código necesario para la presentación de información en el display, y la librería LCD_characters.h que incluye sólo los iconos utilizados en la práctica; con ello conseguimos una estructura modular más flexible que facilita entre otros el análisis de código, su depuración, la corrección de errores y la implementación de nuevas funciones. 
- Se ha eliminado el parpadeo del display LCD a través de la función void initDisplayValues() de la librería LCD.cpp presentando las distintas mediciones por orden sin sobreescribirse constantemente, para ello hemos utilizado un contador que genera una secuencia, y definido un tiempo mínimo de presentación de información de 2s, mientras se ejecuta el loop principal a 500 ms.
